...
for var_name, var_value in REQUIRED_VARS.items():
    if not var_value:
        raise ValueError(f"❌ ENV obrigatória ausente: {var_name}")
...
class FakeHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        try:
            ...
            self.end_headers()
            self.wfile.write(b"BOT V75.2 ONLINE")
        except:
            pass
                    ...
                    for b in event.get('bookmakers', []):
                        for m in b.get('markets', []):
                            for o in m.get('outcomes', []):
                                if o.get('name') == home:
                                    try:
                                        all_h.append({"p": float(o.get('price', 0)), "b": b.get('title', 'Unknown')})
                                    except:
                                        pass
...
async def handle_request(u: Update, c, mode="soccer", is_multi=False):
    msg = await u.message.reply_text("🔎 Buscando jogos...")

    result = await engine.get_matches(mode)
    data = result["data"]

    if not data:
        return await msg.edit_text("❌ Nenhum jogo VIP hoje.")

    if is_multi:
        valid = [g for g in data if g['odd'] > 1.0]
        if len(valid) < 2:
            return await msg.edit_text("❌ Poucos jogos para múltipla.")
