...
# ================= FALLBACK ANTI-VEXAME =================
STAR_PLAYERS = {
    "ARSENAL": "Bukayo Saka", "INTER": "Lautaro Martínez", "MILAN": "Rafael Leão",
    "REAL MADRID": "Vinícius Júnior", "CITY": "Haaland", "LIVERPOOL": "Salah",
    "PSG": "Bradley Barcola", "BAYERN": "Harry Kane", "NAPOLI": "Kvaratskhelia",
    "BARCELONA": "Lewandowski", "FLAMENGO": "Pedro", "PALMEIRAS": "Raphael Veiga",
    "SPORTING": "Gyökeres", "CHELSEA": "Cole Palmer", "NEWCASTLE": "Alexander Isak",
    "ATLETICO MADRID": "Griezmann", "JUVENTUS": "Vlahovic", "CORINTHIANS": "Yuri Alberto",
    "SAO PAULO": "Calleri", "BOTAFOGO": "Igor Jesus", "VASCO": "Vegetti"
}

def get_star_fallback(team_name):
    t_upper = team_name.upper()
    for key, player in STAR_PLAYERS.items():
        if key in t_upper:
            return player
    return None
...
async def get_players_for_matches(jogos_list):
    ...
    {{
        ...
        "Wolverhampton Wanderers x Arsenal": "Bukayo Saka"
    }}
    ...
    try:
        ...
        logging.error(f"❌ Erro na IA (Usando Fallback Local): {e}")
        return {}
...
def analyze_game(game, player_star):
    if not player_star:
        player_star = get_star_fallback(game["home"]) or get_star_fallback(game["away"])
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.send_response(200); self.end_headers(); self.wfile.write(b"ONLINE - DVD TIPS V180")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🦁 <b>BOT V180 ONLINE (Trava de Hoje Ativa)</b>", reply_markup=get_main_menu(), parse_mode=ParseMode.HTML)
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "fut":
        await q.message.reply_text("⏳ <b>Buscando as rodadas EXATAS de hoje e analisando artilheiros...</b>", parse_mode=ParseMode.HTML)
