...
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY") # NOVA CHAVE
...
# ================= BANCO DE DADOS =================
def load_db():
    default = {"users": {}, "keys": {}, "tips_history": [], "last_run": "", "api_cache": None, "api_cache_time": None}
...
def start_web_server():
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self): self.send_response(200); self.end_headers(); self.wfile.write(b"DVD TIPS AI V6 ON")
...
def get_ai_analysis(match, tip):
    """Pede ao Gemini uma justificativa curta para a aposta"""
    if not GEMINI_API_KEY:
        return "Análise baseada em estatísticas recentes e momento das equipes."
    
    try:
        model = genai.GenerativeModel('gemini-1.5-flash') # Modelo rápido e barato
        prompt = f"""
        Atue como um analista de apostas esportivas profissional e direto.
        Jogo: {match}
        Minha Tip: {tip}
        
        Escreva UMA frase curta (max 15 palavras) justificando essa tip com base em conhecimentos gerais de futebol. 
        Seja técnico mas empolgante. Não use aspas.
        """
        response = model.generate_content(prompt)
        return response.text.strip()
    except Exception as e:
        logger.error(f"Erro Gemini: {e}")
        return "Tendência forte baseada no histórico recente."

async def ask_ai_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Comando /analisar para usuários perguntarem ao bot"""
    ...
    try:
        ...
        prompt = f"Faça uma análise rápida de apostas para o jogo: {user_msg}. Dê um palpite de vencedor e gols. Seja curto."
        response = model.generate_content(prompt)
...
def get_real_matches(force_refresh=False):
    ...
    try:
        ...
        for game in data:
            ...
            if game_time_utc > now + timedelta(hours=24): continue
            
            time_str = (game_time_utc - timedelta(hours=3)).strftime("%H:%M")
            ...
            if h2h:
                ...
                fav = odds[0]
                if 1.30 <= fav['price'] <= 2.20: tip, odd = f"Vence {fav['name']}", fav['price']
            ...
            if tip:
                # Chama o Gemini AQUI para gerar a justificativa na hora
            ...
            if len(matches) >= 10: break # Reduzi para 10 para não estourar tempo da IA
...
def generate_simulated_matches():
    ...
    for _ in range(5):
        ...
        tip = "Over 2.5 Gols"
        matches.append({
            "match": f"{t1} x {t2}", "tip": tip, "odd": 1.80, 
            "league": "Simulado", "time": "19:00",
...
def generate_multiple_bet(matches_pool):
    ...
    return {"match": "🔥 BILHETE PRONTO DO DIA 🔥", "tip": "\n".join(desc), "odd": round(multi_odd, 2)}
    ...
    for uid in db["users"]:
        try:
            ...
            for tip in selection:
                # Inclui a justificativa da IA (tip['reason'])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🤖 **DVD TIPS AI V6.0**\nUse `/analisar [jogo]` para pedir dicas à IA!", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")

async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not check_admin(update.effective_user.id): return
    kb = [[InlineKeyboardButton("🚀 Enviar Tips (AI)", callback_data="force_tips")], [InlineKeyboardButton("🔑 Gerar Chave", callback_data="gen_key")]]
...
if __name__ == "__main__":
    ...
    app.add_handler(CommandHandler("analisar", ask_ai_command)) # NOVO COMANDO
    ...
    app.add_handler(ConversationHandler(entry_points=[CallbackQueryHandler(enter_key, pattern="^enter_key$")], states={1: [MessageHandler(filters.TEXT, process_key)]}, fallbacks=[]))
    
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    print("🤖 DVD TIPS AI V6.0 - ONLINE")
